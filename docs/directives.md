...
`vln-if`, `vln-loop`, `vln-route`, and `vln-fragment` can play CSS
transitions on mount and unmount by tagging the element with two class
names:

- `.vln-entering` — added on mount, stripped after two animation frames.
- `.vln-leaving` — added on unmount, held on the element for the CSS
  `transition-duration` (or `animation-duration`) plus a 50 ms slack,
  then removed. For multi-property transitions the longer duration
  wins — the node is held until the last one would have finished.

Write CSS against those class names — the library only owns the timing.
Elements with no `transition` or `animation` unmount synchronously.

**Requires the `velin-transitions` module.** `@velinjs/all` bundles it
automatically. Users pulling individual bundles (`velin-standard`,
`velin-templates`, `velin-router`) need to also load
`velin-transitions.min.js` (or import `@velinjs/all/transitions`) —
without it, the four directives fall back to synchronous removal and
these class names are never added. `velin-common` (core + std) omits
transitions on purpose to stay lean.
...
<div vln-if="visible" class="fade">Fades in and out</div>
```

For list transitions, style the item template:

```html
<style>
  .item { transition: opacity 250ms, transform 250ms; }
  .item.vln-leaving { opacity: 0; transform: translateX(16px); }
</style>

<li vln-loop:item="items" class="item" vln-text="item"></li>
```

Notes:
- Put the `transition` on the toggled element itself, not on its
  descendants — the library reads computed styles from that one node.
- **`vln-if` and `vln-route`: revive on re-toggle.** If you flip back
  before the leave finishes, the *same node* is revived — the leave is
  canceled, the `vln-leaving` class is stripped, and the CSS transition
  reverses back to natural state. Bindings on the revived node stay
  intact.
- **`vln-fragment` and `vln-route`: out-in swaps.** A template swap or
  route change waits for the outgoing subtree to finish leaving before
  the new one mounts — no overlap. Rapid successive swaps collapse to
  the latest target: the intermediate targets never render. On
  `vln-fragment`, reverting to the outgoing template mid-leave revives
  it (same-node cancel-and-restore).
- **`vln-loop`: leaving rows animate in place.** With a keyed loop, when
  you remove an item from the middle, the actual removed row gets
  `vln-leaving` and animates out from its original slot; other rows do
  not shift until the leaving row is finally removed. Rapid updates
  fast-forward any still-leaving ghosts so they don't accumulate.
- **Middle-removal on lists — use a keyed loop.** Positional
  `vln-loop:item="items"` reuses DOM slots by index, so removing an item
  from the middle rewrites every subsequent row in place and animates
  the *trailing* slot leaving. That's almost never what you want. Use
  `vln-loop:item="{collection: items, key: 'id'}"` — the actual removed
  row animates, everything else stays put.
- The class contract is the whole feature. Anything more elaborate
  (staggering, orchestration, JS-driven animation) is out of scope — reach
  for a dedicated animation library.
